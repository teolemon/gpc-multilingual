#!/usr/bin/env python3
"""
Script to create a better formatted multilingual taxonomy file.
"""

import os
import csv
import re
import json
from collections import defaultdict, OrderedDict
from pathlib import Path


def parse_taxonomy_file(file_path):
    """Parse a single taxonomy file and extract category data."""
    categories = {}
    
    with open(file_path, 'r', encoding='utf-8') as f:
        for line in f:
            line = line.strip()
            if not line or line.startswith('#'):
                continue
                
            # Parse format: "ID - Category > Subcategory > ..."
            match = re.match(r'^(\d+)\s*-\s*(.+)$', line)
            if match:
                category_id = int(match.group(1))
                category_path = match.group(2)
                categories[category_id] = category_path
    
    return categories


def parse_query_csv(file_path):
    """Parse the query.csv file to extract wikidata mappings."""
    wikidata_mappings = {}
    
    with open(file_path, 'r', encoding='utf-8') as f:
        reader = csv.reader(f)
        next(reader)  # Skip header
        
        for row in reader:
            if len(row) >= 3:
                wikidata_uri = row[0]
                label = row[1]
                try:
                    category_id = int(row[2])
                    wikidata_mappings[category_id] = {
                        'uri': wikidata_uri,
                        'label': label
                    }
                except ValueError:
                    # Skip malformed entries where category_id is not a number
                    continue
    
    return wikidata_mappings


def extract_language_code(filename):
    """Extract language code from filename like 'taxonomy-with-ids.en-US.txt'"""
    match = re.search(r'taxonomy-with-ids\.([^.]+)\.txt$', filename)
    return match.group(1) if match else None


def build_hierarchy_structure(categories):
    """Build a hierarchical structure from category paths."""
    hierarchy = {}
    
    for category_id, path in categories.items():
        parts = [part.strip() for part in path.split(' > ')]
        
        # Store the full path and its depth
        hierarchy[category_id] = {
            'path': parts,
            'depth': len(parts),
            'full_path': path
        }
    
    return hierarchy


def create_formatted_multilingual_taxonomy():
    """Create a nicely formatted multilingual taxonomy file."""
    
    # Get all taxonomy files
    taxonomy_files = list(Path('.').glob('taxonomy-with-ids.*.txt'))
    
    # Parse all taxonomy files
    all_languages = {}
    all_category_ids = set()
    
    print(f"Found {len(taxonomy_files)} taxonomy files")
    
    for file_path in taxonomy_files:
        lang_code = extract_language_code(file_path.name)
        if not lang_code:
            continue
            
        print(f"Parsing {file_path.name} (language: {lang_code})")
        categories = parse_taxonomy_file(file_path)
        all_languages[lang_code] = categories
        all_category_ids.update(categories.keys())
        print(f"  Found {len(categories)} categories")
    
    # Parse wikidata mappings
    print("\nParsing query.csv for wikidata mappings...")
    wikidata_mappings = parse_query_csv('query.csv')
    print(f"Found {len(wikidata_mappings)} wikidata mappings")
    
    # Use English as the base structure for hierarchy
    base_lang = 'en-US'
    if base_lang not in all_languages:
        print(f"Warning: Base language {base_lang} not found, using first available language")
        base_lang = list(all_languages.keys())[0]
    
    base_hierarchy = build_hierarchy_structure(all_languages[base_lang])
    
    # Sort categories by depth then by ID for proper hierarchical order
    sorted_categories = sorted(
        all_category_ids,
        key=lambda cat_id: (
            base_hierarchy.get(cat_id, {}).get('depth', 999),
            cat_id
        )
    )
    
    # Create the nicely formatted output
    output_lines = []
    output_lines.append("# Multilingual Google Product Taxonomy")
    output_lines.append("# Generated from monolingual files with wikidata mappings")
    output_lines.append("# ")
    output_lines.append("# Format for each entry:")
    output_lines.append("# =====================")
    output_lines.append("# Category ID: [number]")
    output_lines.append("# Hierarchy Level: [depth]")
    output_lines.append("# Wikidata: [URI] ([label])")
    output_lines.append("# Languages:")
    output_lines.append("#   [lang]: [category_path]")
    output_lines.append("#   ...")
    output_lines.append("# ")
    output_lines.append(f"# Statistics:")
    output_lines.append(f"# - Total categories: {len(sorted_categories)}")
    output_lines.append(f"# - Languages: {len(all_languages)} ({', '.join(sorted(all_languages.keys()))})")
    output_lines.append(f"# - Categories with Wikidata: {len(wikidata_mappings)}")
    output_lines.append("")
    output_lines.append("=" * 80)
    output_lines.append("")
    
    for i, category_id in enumerate(sorted_categories):
        # Get hierarchy info from base language
        hierarchy_info = base_hierarchy.get(category_id, {})
        depth = hierarchy_info.get('depth', 0)
        
        # Get wikidata mapping if available
        wikidata_info = wikidata_mappings.get(category_id, {})
        wikidata_uri = wikidata_info.get('uri', 'N/A')
        wikidata_label = wikidata_info.get('label', 'N/A')
        
        # Start the entry
        output_lines.append(f"Category ID: {category_id}")
        output_lines.append(f"Hierarchy Level: {depth}")
        
        if wikidata_uri != 'N/A':
            output_lines.append(f"Wikidata: {wikidata_uri} ({wikidata_label})")
        else:
            output_lines.append("Wikidata: Not available")
        
        output_lines.append("Languages:")
        
        # Collect and sort translations
        for lang_code in sorted(all_languages.keys()):
            if category_id in all_languages[lang_code]:
                category_name = all_languages[lang_code][category_id]
                output_lines.append(f"  {lang_code}: {category_name}")
        
        output_lines.append("")
        
        # Add progress indicator every 100 entries
        if (i + 1) % 100 == 0:
            print(f"Processed {i + 1}/{len(sorted_categories)} categories...")
    
    # Write the output file
    output_file = 'multilingual-taxonomy-formatted.txt'
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write('\n'.join(output_lines))
    
    print(f"\nCreated formatted multilingual taxonomy file: {output_file}")
    
    # Also create a JSON version for programmatic use
    json_data = {
        'metadata': {
            'total_categories': len(sorted_categories),
            'languages': sorted(all_languages.keys()),
            'wikidata_mappings': len(wikidata_mappings)
        },
        'categories': {}
    }
    
    for category_id in sorted_categories:
        hierarchy_info = base_hierarchy.get(category_id, {})
        wikidata_info = wikidata_mappings.get(category_id, {})
        
        translations = {}
        for lang_code in sorted(all_languages.keys()):
            if category_id in all_languages[lang_code]:
                translations[lang_code] = all_languages[lang_code][category_id]
        
        json_data['categories'][str(category_id)] = {
            'hierarchy_level': hierarchy_info.get('depth', 0),
            'wikidata_uri': wikidata_info.get('uri'),
            'wikidata_label': wikidata_info.get('label'),
            'translations': translations
        }
    
    json_file = 'multilingual-taxonomy.json'
    with open(json_file, 'w', encoding='utf-8') as f:
        json.dump(json_data, f, ensure_ascii=False, indent=2)
    
    print(f"Created JSON version: {json_file}")


if __name__ == "__main__":
    create_formatted_multilingual_taxonomy()